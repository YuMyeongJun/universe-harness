...
#   · **진짜 원격 주소**(`https://` · `git@`). 네트워크를 안 쓰기로 했으니 **자격·리다이렉트·
#     인증 실패의 진짜 모양**은 여기서 안 재진다. 재는 것은 `file://` 로 되는 데까지다.
#   · **큰 저장소**. 시간 제한·부분 클론은 못 본다.
#   · **초안이 옳은가.** 「TODO 가 남았나 · 좌표와 목록이 같이 움직이나」만 본다 —
#     그 좌표로 잰 수가 **맞는 수인지**는 관측이 판단하고, 그 판단은 사람이 본다.
set -uo pipefail
cd "$(dirname "$0")/.."
