...
export interface IHarnessCommands {
  ...
  typecheck?: string;
  /**
   * 브라우저에서 실제로 도는지 보는 명령(E2E).
   * ⚠️⚠️ 게이트가 재는 것은 「별이 **선다**」까지다 — **사람이 볼 수 있는지는 안 본다.**
   * 이 저장소가 세 자리에 그렇게 적어 두고도 재지 않던 축이다.
   * ⛔ **브라우저를 우리가 깔지 않는다.** 은하가 자기 명령을 적을 때만 돈다 —
   *    안 적으면 ⚪ 「못 쟀다」다. 명령을 지어내면 없는 E2E 가 통과로 세어진다(R146).
   */
  e2e?: string;
  /** 추가 정적 검사. 이름 → 명령. */
