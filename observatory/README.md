...
재는 대상이 셋이라 장치도 셋이다. 셋은 서로를 대신하지 못한다 — 무엇을 고쳤는지에 따라
부르는 것이 달라진다.
...
우주 저장소를 직접 고치는 중이라 명령이 아직 안 이어져 있으면, `universe X` 를
`node <저장소>/bin/universe.mjs X` 로 바꿔 치면 같다.
...
엔진은 `observatory/engine` 에 **벤더링**돼 있다. 정본은 `fe-agent-harness` 저장소이고
여기 있는 것은 그 복사본이므로 **자산이 아니라 산출물**이다(보존 법칙 §자산 vs 산출물).
그래서 `verify-laws.sh` 가 우주의 문서 법칙에서 제외한다.

```bash
cd observatory/engine && npm install && npm run build && npm run selftest
```

엔진의 경로는 `universe.config.json` 의 `observatory.path` 가 들고 있다.

## 함정

- ⛔ **게이트가 빨간불이면 채점하지 않는다.** 죽은 빌드 위의 수치는 전부 거짓이다.
- ⛔ **파이프 뒤에서 판정하지 않는다**(관측 법칙 §3). 판정이면 파일로 떨구고
  (`cmd > log 2>&1; echo "exit=$?"`) 그다음에 걸러라.
- **`observatory.path` 에 절대경로를 박지 마라.** 그 값은 **우주 루트 기준 상대경로**이고
  (`observatory/engine`) 스크립트가 `path.resolve(root, …)` 로 푼다. 절대경로를 박으면
  이 우주는 그 노트북에서만 돈다.
- 게이트는 **중앙에서 한 번만** 돌린다. 타입 인지 lint 저장소에서 동시에 돌리면
  10.3초짜리가 21분이 된다.
...
이 기관이 우주와 정원이 갈리는 지점인 이유가 그것이다.
