...
export interface IGalaxyDraftTodos {
   ...
   * ⛔ **값이 아니라 설명이다** — 화면은 칸 **옆에** 보여 줄 뿐 칸 **안에** 넣지 않는다.
   *   넣는 순간 짐작한 값이 사람이 적은 값 행세를 한다.
   * ⚠️ 도구가 아무 말도 안 남긴 자리는 빈 문자열이다 — 그때 화면은 「도구가 말을 안 남겼다」고
   *   말해야 한다. ⛔ 그럴듯한 설명을 화면이 지어내면 그것도 짐작이다.
   */
