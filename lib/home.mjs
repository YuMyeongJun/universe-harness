 ...
 * 설치된 상태에서는 이렇게 갈린다:
 *
 *   node_modules/universe-harness/   ← 패키지의 집 (씨앗 · 엔진 · 스크립트)
 *   <소비 저장소>/universe/           ← 우주의 집 (법칙 · 은하 · 성운 · 로그)
 *
 * ⛔ **두 줄의 `universe` 는 서로 다른 것이다.** 위는 **패키지 이름**(`npm i <여기>`)이라
 *    `universe-harness` 이고, 아래는 `init` 이 만드는 **폴더 이름**이라 `universe` 그대로다
  ...
  console.error(
    [
      ...
      /* ⛔⛔ **이 경고는 이름이 바뀌어도 그대로 유효하다 — 오히려 더 필요해졌다.**
         ...
         이 패키지는 `universe-harness` 라는 **다른 이름**으로 올라간다. 그런데 **명령 이름**은
         ...
         **밖에서 부르면 남의 것을 내려받는다** — 같은 글자가 자리에 따라 다른 것을 가리킨다.
         ...
         ⚠️ 이 줄에 `npx universe-harness …` 를 적지 마라 — 부품 시험의 `npx\s+universe\b`
         감시가 그것도 「남의 패키지를 시키는 자리」로 물어 버린다(정규식이 `-` 를 경계로 본다). */
      '    (⛔ `npx universe` 가 아니다 — npm 의 그 이름은 남의 패키지(crossfilter/universe)다.',
      '     이 패키지의 **npm 이름**은 `universe-harness` 이고, **명령 이름**만 `universe` 다)',
