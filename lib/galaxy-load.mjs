...
/**
 * 은하 좌표가 사는 곳 — **로컬 명부가 먼저다.**
 *
 * ⚠️⚠️ 왜 둘인가(R152). `galaxies/` 는 **커밋된다.** 그래서 거기에는
 * **그 기계에만 있는 좌표를 둘 수 없다** — 공개 저장소에 남의 홈 경로가 실려 나간다
 * (실측: `galaxies/backoffice.json` 의 `/Users/…/orca/…` 가 GitHub 에 올라가 있었다).
 *
 * ⛔ 그렇다고 명부를 통째로 저장소 밖으로 뺄 일은 아니다. **픽스처 은하**
 * (`tiny-galaxy`·`messy-galaxy`)는 상대 경로라 좌표가 아니고, 우주가 자기를 시험하는 데 쓴다.
 * ⇒ 가르는 기준은 **소속이 아니라 경로의 종류**다: 상대 경로는 커밋되고, 절대 경로는 로컬에 남는다.
 * 그 규칙을 `observatory/verify-coordinates.mjs` 가 무는다 — 회사 이름을 열거하지 않고 **구조로** 잰다.
 */
export const GALAXY_DIRS = ['galaxies.local', 'galaxies'];

/**
 * 좌표 파일이 실제로 있는 자리를 준다. **로컬 → 커밋본** 순서.
 * ⛔ 자리를 찾는 규칙이 두 곳에 생기면 조용히 갈린다 — R143 이 그것으로 물렸다
 * (좌표를 직접 읽는 자리가 다섯이었고 상대 경로가 한 번 더 붙었다).
 * @returns {Promise<string|null>} 없으면 `null`
 */
export const findGalaxyFile = async (root, name) => {
  for (const dir of GALAXY_DIRS) {
    const candidate = path.join(root, dir, `${name}.json`);
...
export const loadGalaxy = async (root, name, registered = []) => {
  /* 로컬 명부에 있으면 그것이 이긴다 — 등재도 로컬이어야 하므로 `registered` 를 안 본다. */
  const file = (await findGalaxyFile(root, name)) ?? path.join(root, 'galaxies', `${name}.json`);
  let text;
  try {
