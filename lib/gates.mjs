...
export const GATES = [
  ...
  /* ⛔ **브라우저를 안 띄운다** — 커밋되는 리포트 픽스처로 「셋으로 갈리는가」만 잰다.
     진짜 e2e 주행은 은하가 `commands.e2e` 를 선언한 뒤 사람이 돌린다(§8). */
...
export const NOT_GATED = {
  ...
  /**
   * ⛔ 관문은 이 둘을 **탐침(`probe-loop.sh`)으로** 돌린다 — 직접 등재하면 축을 선언한
   * 은하에서 **매 바퀴 브라우저를 띄운다.** 느린 관문은 아무도 안 본다.
   * ⚠️ 2026-09-09 에 이 사유가 **거짓이었던 때가 있었다** — 탐침이 TC 도구와 함께 나갔는데
   * 문장만 남아 「탐침이 잰다」고 말했다(같은 키가 두 번 적혀 정직한 쪽이 덮였다).
   * 지금은 탐침이 커밋되는 픽스처로 일곱 갈래를 재고, `NOT_GATED` 중복 키를 부품 시험이 문다.
   */
  'observatory/loop-state.mjs':
    '탐침(probe-loop.sh)이 일곱 갈래를 재고, 실제 e2e 주행은 은하가 `commands.e2e` 를 선언한 뒤 사람이 돌린다',
  'observatory/repeat.mjs':
    '같은 탐침이 멈추는 자리를 재고, 실제 반복은 사람이 부른다(브라우저를 띄운다)',
