 ...
 *
 * ## ⛔ 이름이 바뀌어도 이 판별은 안 틀린다 — **이름으로 안 재기 때문이다**
 *
 * npm 패키지 이름은 `universe-front-harness` 이고 **명령 이름은 `universe`** 다(`bin` 키).
 * 그래서 PATH 에서 찾는 것은 `universe` 그대로다 — 여기를 `universe-front-harness` 로 바꾸면
 * 있는 명령을 「없다」고 말한다.
 * ⚠️ 그리고 「그것이 **우리 것인가**」는 **이름이 아니라 실경로**로 판별한다
 * (`resolved === ${packageHome}/bin/universe.mjs`). 남의 `universe`(crossfilter/universe)가
 * PATH 에 있어도 이름은 똑같으므로 **이름으로는 영영 못 가른다** — R161 이 그 사건이다.
 * ⇒ 패키지 이름이 무엇으로 바뀌든 이 함수는 손댈 자리가 없다.
 *
