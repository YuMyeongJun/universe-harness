...
if (!isUniverseSource) {
  ...
  const MUST_IGNORE = [
    ...
    /* ⛔ 실측: 은하가 낸 화면 시험 리포트(`qa/.universe/e2e.json`)가 **커밋 대상으로 올라왔다.**
       `test-results/` 만 막혀 있었는데 `universe loop` 가 읽는 자리를 `.universe/` 로 정하면서
       **방어가 안 따라왔다** — 옮긴 자리에 방어가 안 따라오는 그 형태다(R162). */
    ['qa/.universe/e2e.json', '화면 시험 리포트 — URL·계정·스크린샷 경로가 섞여 온다'],
    ['app/universe/.universe/e2e.json', '다른 은하도 같은 자리를 쓴다'],
  ];
  const asked = [];
