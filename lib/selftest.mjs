...
/**
 * ⛔ **사람이 읽는 `.md` 를 전부 훑는다 — 폴더를 손으로 열거하면 그 밖은 영영 안 보인다(§9).**
 *
 * ⚠️ 실측: 아래 두 검사(「남의 패키지를 시키는가」·「수치를 손으로 적었는가」)가
 * `docs/` 와 루트 둘만 보고 있었다. 그래서 **배달되는 기관 README**
 * (`bigbang/README.md` · `observatory/README.md` …)에 `npx universe` 와
 * 「관문 33개」를 넣어도 **둘 다 안 물었다**(변이 둘로 확인 — ❌ 놓쳤다).
 * 소비 저장소 사람이 읽는 글이라 거기 적힌 거짓이 그대로 배달된다.
 *
 * ⛔ 빼는 것은 **역사와 남의 것**뿐이다 — `log/`(그때의 기록) · `redshift/`(판올림 기록) ·
 * `observatory/engine`(남의 저장소) · `node_modules`.
 */
const SKIP_DOC_DIRS = ['node_modules', '.git', 'engine', 'log', 'redshift'];
const walkDocs = async (dir = '.') => {
  const out = [];
  for (const entry of await readdir(join(ROOT, dir), { withFileTypes: true }).catch(() => [])) {
    const rel = dir === '.' ? entry.name : `${dir}/${entry.name}`;
    if (SKIP_DOC_DIRS.includes(entry.name)) {
      continue;
    }
    out.push(...(entry.isDirectory() ? await walkDocs(rel) : rel.endsWith('.md') ? [rel] : []));
  }
  return out;
};
/**
 * ⛔ **git 이 무시하는 자리를 분모에 넣으면 안 된다.** 폴더를 걸어서 훑었더니
 * `app/universe/.data/clones/*` 의 **남의 저장소 사본 여섯 편**이 잡혔다 —
 * 그 저장소들의 문장을 우리 규율로 판정하는 꼴이고, 사본을 하나 받으면 늘고 지우면 준다.
 * ⇒ **git 에게 묻는다**(R163 이 「못 읽는 비율」에서 쓴 것과 같은 방법).
 * ⚠️ 배달본에는 git 이 없다 — 그때는 걸어서 훑는다(거기엔 `.data` 자체가 없다).
 */
let docCache = null;
const collectDocs = async () => {
  ...
  const listed = tracked.status === 0
    ? tracked.stdout.split('\0').filter(Boolean)
      .filter((f) => !SKIP_DOC_DIRS.some((d) => f === d || f.startsWith(`${d}/`) || f.includes(`/${d}/`)))
    : await walkDocs();
  docCache = listed;
  return docCache;
};
...
if (docs.length === 0) {
  ...
  const bad = [];
  const scannedDocs = await collectDocs();
  for (const file of [...scannedDocs, ...codeFiles]) {
    const text = await readFile(join(ROOT, file), 'utf8');
    for (const [index, line] of text.split('\n').entries()) {
      /* ⛔ **`\b` 는 우리 이름까지 문다.** `universe\b` 는 `universe-front-harness` 의
         `universe` 뒤(하이픈 앞)에서도 끊겨서, **정상 사용법을 적은 줄이 빨간불**이 됐다 —
         지금까지는 그 줄들에 우연히 「치지 마라」가 붙어 있어 도피구로 지나가고 있었다.
         ⇒ 뒤에 `-` 나 낱말이 붙으면 **다른 패키지 이름**이므로 안 본다.
         경고문 자체는 그 문자열을 품어야 하므로 「치지 마라」가 붙은 줄은 그대로 뺀다. */
      /* ⚠️ 도피구가 **한국어 낱말만 알아서** 영어 경고문(`README.en.md` 의
         「Never type `npx universe`」)을 **오탐했다** — 훑개를 저장소 전체로 넓히자마자
         드러났다. 영어 짝을 같이 둔다. ⛔ 이건 하한이다: 새 언어를 더하면 여기도 는다. */
      if (/npx\s+universe(?![-\w])/.test(line) && !/치지 마라|가 아니다|Never type|belongs to someone else/.test(line)) {
  ...
  console.log(`✅ 남의 패키지를 시키는 자리가 없다 (문서 ${scannedDocs.length}편 + 코드 ${codeFiles.length}개)`);
}
...
if (!isUniverseSource) {
  ...
  const MUST_IGNORE = [
    ...
    ['node_modules/x.js', '설치물은 커밋하지 않는다'],
    /* ⛔ 실측: 은하가 낸 화면 시험 리포트(`.universe/e2e.json`)가 **커밋 대상으로 올라왔다.**
       `test-results/` 만 막혀 있었는데 `universe loop` 가 읽는 자리를 `.universe/` 로 정하면서
       **방어가 안 따라왔다** — 옮긴 자리에 방어가 안 따라오는 그 형태다(R162).
       ⚠️ 2026-09-09: 화면이 `feat/console-screens` 로 나가면서 `app/`·`qa/` 를 겨누던 줄들을
       뺐다. **막는 규칙(`.gitignore`)은 그대로 둔다** — 화면이 돌아올 때 방어가 먼저 있어야 한다.
       ⛔ 여기서 재던 것을 지운 것이지 방어를 지운 것이 아니다. */
    ['.universe/e2e.json', '화면 시험 리포트 — URL·계정·스크린샷 경로가 섞여 온다'],
    ['.auth/storageState.json', '**로그인된 세션 그 자체**다 — 계정 정보보다 무겁다'],
    ['test-results/x.png', 'Playwright 산출물 — 스크린샷이 들어간다'],
    ['trace.zip', '트레이스 — 스크린샷과 네트워크 헤더가 들어간다'],
  ];
  const asked = [];
...
if (isUniverseSource) {
  ...
  /* ⚠️ 예전엔 `console` 은하였다. 2026-09-09 에 화면이 `feat/console-screens` 로 나가면서
     그 이름이 사라졌고, 이 검사는 「은하가 하나도 없다」로 죽었다 — **옳게 죽었다**(§8:
     0은 무죄가 아니다). ⇒ 늘 있는 픽스처로 옮긴다. `messy-galaxy` 는 **일부러 더러운** 은하라
     위반이 0이 아니고, 그래서 「빈 목록」과 「깨끗함」을 이 검사가 실제로 가를 수 있다. */
  const plain = await run(['--galaxy', 'messy-galaxy']);
  const asJson = await run(['--galaxy', 'messy-galaxy', '--json']);
...
/**
 * **도구가 화면에 적는 처방이 실재하는가** — 아무도 안 재던 자리다.
 *
 * ⛔⛔ 우리는 **검사만** 변이시켜 왔다. 그런데 도구가 **사람에게 하는 말**은 아무도 안 쟀다.
 * ⚠️ 실측(옆 저장소 세션이 이 축을 열었다): 그쪽 도구가 「같은 줄에 날짜를 적으면 면제됩니다」라고
 * **약속**해 놓고 검사 하나에서만 그 면제를 봤다 — **시키는 대로 해도 안 풀렸다.**
 * 사람은 그것을 「검사가 고장났다」로 읽고 **다음엔 안 본다.**
 *
 * ⚠️ 여기서도 셋이 거짓이었다. 도구가 자기를 이렇게 불렀는데 **셋 다 없는 명령**이었다:
 *     `blind-census` 라고 자기를 불렀다(진짜는 `census`) · `universe coordinates` · `universe qa`
 * 플래그를 잘못 주면 「⛔ universe blind-census: 모르는 플래그」가 났고, 그대로 치면
 * 「모르는 명령: blind-census」가 났다. **도구가 자기 이름을 틀리게 말했다.**
 *
 * ⛔ 처방 목록을 여기 적지 않는다 — 소스에서 **뽑아서** 배분표와 대조한다(§9).
 * ⚠️ 그리고 **질문을 답에서 만들지 않는다**(순환): 후보는 `git ls-files` 로 받은 **도구 전부**에서
 *    뽑고, 정답은 `bin/universe.mjs` 의 배분표다. 두 자리가 독립이다.
 */
{
  /* ⛔ **「git 이 답을 안 했다」와 「답이 비었다」를 가른다** — 뭉치면 배달본이 「고장」이 된다. */
  const asked = await new Promise((resolve) => {
    /* ⛔ **아직 커밋 안 된 새 도구도 본다.** 실측: `learn-cards.mjs` 가 「없는 명령을 치라」고
       말하고 있었는데, **`git add` 하기 전까지 이 검사가 못 봤다** — 새 도구는 만들어질 때
       가장 틀리기 쉬운데 바로 그때가 검사 밖이었다. `--others --exclude-standard` 로 덮는다
       (⛔ `--ignored` 는 안 준다 — 버려질 것까지 세면 분모가 흔들린다). */
    const child = spawn('git', ['ls-files', '--cached', '--others', '--exclude-standard', 'bin', 'observatory', 'lib'],
      { cwd: ROOT, stdio: ['ignore', 'pipe', 'ignore'] });
    let out = '';
    child.stdout.on('data', (d) => { out += d; });
  ...
  const tools = (asked ?? []).filter((f) => f.endsWith('.mjs'));
  /**
   * ⛔ **「git 이 없다」와 「훑개가 고장났다」를 가른다** — 실측으로 배웠다.
   *
   * 배달본은 **git 저장소가 아니다.** 처음엔 분모 가드가 거기서 「훑개가 고장 났다」로 죽었고,
   * `verify-delivery` 가 **빨간불**을 냈다. 배달본에서 못 재는 것은 **고장이 아니라 ⚪** 다.
   * ⚠️ 그렇다고 가드를 지우면 **우주 저장소에서 훑개가 죽어도 조용해진다.** ⇒ 갈라 쓴다:
   *   **git 이 답을 안 했으면**(종료코드≠0) 못 쟀다 · **답이 비었으면** 훑개 고장이다.
   * ⛔ 첫 갈림점으로 `universe.config.json` 의 유무를 썼는데 **틀렸다** — 배달본에도 그 파일이 있다.
   */
  if (asked === null) {
    console.log('⏭  도구가 적는 처방 — **git 에게 못 물었다**(배달본은 저장소가 아니다). 못 쟀다.');
  } else {
    assert.ok(tools.length > 10, `도구를 ${tools.length}개만 봤다 — 훑개가 고장 났다(§8)`);

    const manifest = await readFile(join(ROOT, 'bin/universe.mjs'), 'utf8');
    const known = new Set([...manifest.matchAll(/^\s{2}'?([a-z-]+)'?:\s*'[^']+'/gm)].map((m) => m[1])  /* ⛔ 따옴표 붙은 키(`'learn-cards':`)도 읽는다 — 하이픈이 든 이름은 JS 가 따옴표를 요구한다.
       실측: 그것 때문에 **배분표에 있는 명령을 「없다」고** 말했다(훑개가 먼저 틀린 자리). */);
    known.add('help');
    assert.ok(known.size > 20, `배분표를 ${known.size}개로 읽었다 — 훑개가 고장 났다(§8)`);

    const lies = new Map();
    let advised = 0;
    /**
     * ⛔ **변이 명부만 뺀다 — 그 파일의 일은 「일부러 틀린 문자열」을 담는 것이다.**
     * 실측: 「도구가 없는 명령을 말한다」를 등재하자 그 등재문(`'universe not-a-command'`)을
     * 이 검사가 물었다 — **일부러 심은 가짜를 진짜 처방으로 읽었다.**
     * ⚠️ 빼는 것은 **조용해지는 방향**이라 값이 있다. 그래서 **한 파일만**, 이유를 적고 뺀다 —
     *    나머지 도구 60여 곳은 그대로 본다. ⛔ 이름 목록으로 늘리지 마라.
     */
    const REGISTRY = 'observatory/verify-checks.mjs';
    for (const file of tools.filter((f) => f !== REGISTRY)) {
      /* eslint-disable-next-line no-await-in-loop */
      const text = await readFile(join(ROOT, file), 'utf8');
      /* ⛔ **주석은 뺀다 — 처방은 주석이 아니라 출력에 산다.**
         실측: 이 검사가 처음 문 것이 **자기 주석**이었다. 옛 이름을 「이렇게 틀리게 말했었다」고
         인용한 자리다. 그 문장을 지우게 만들면 **검사가 역사를 지운다** — R163 에서 겪은 그것이다. */
      const code = text.split('\n').filter((l) => !/^\s*(\*|\/\*|\/\/)/.test(l)).join('\n');
      for (const m of code.matchAll(/universe (-{0,2}[a-z][a-z-]+)/g)) {
        const said = m[1];
        /* `--help`·`--all` 같은 플래그는 명령이 아니다 — 그건 인자 감사가 본다. */
        if (said.startsWith('-')) {
          continue;
        }
        advised += 1;
        if (!known.has(said)) {
          lies.set(said, (lies.get(said) ?? new Set()).add(file));
        }
      }
    }
    assert.ok(advised > 20, `처방을 ${advised}개만 봤다 — 훑개가 고장 났다(§8)`);
    assert.deepEqual([...lies.keys()], [],
      '도구가 **없는 명령을 치라고 말한다** — 시키는 대로 하면 「모르는 명령」이 난다:\n'
      + [...lies].map(([s, where]) => `   universe ${s}  ← ${[...where].join(' · ')}`).join('\n')
      + '\n   ⛔ 도구가 화면에 적은 처방은 **그 도구가 실제로 지키는 것**이어야 한다.\n'
      + '   ⚠️ 안 지키는 처방은 ⚪ 보다 나쁘다 — ⚪ 는 「못 쟀다」지만 이건 **사람을 헛짓으로 보낸다.**');
    console.log(`✅ 도구가 적는 처방 ${advised}개가 전부 실재하는 명령이다 (도구 ${tools.length}개 · 배분표 ${known.size}개)`);

    /**
     * ⛔⛔ **이름이 맞아도 플래그가 거절되면 처방은 죽는다.**
     *
     * 이 우주는 `rejectUnknownFlags` 로 **모르는 플래그를 거부한다**(그게 옳다). 그래서
     * 도구가 `universe observe --sample 8` 이라고 적어 놓고 그 도구가 `--sample` 을 안 받으면
     * 시키는 대로 친 사람은 **⛔ 모르는 플래그** 를 본다 — 이름만 맞은 처방이다.
     *
     * ⚠️ 옆 저장소 세션이 더 안쪽을 짚었다: 그쪽은 **처방과 그 처방을 재는 시험이 두 벌**이라
     * 손으로 베낀 시험에 **플래그 하나가 빠져** 있었고, 관문의 진짜 처방은 한 번도 안 재졌다.
     * ⇒ 여기서는 **처방을 소스에서 뽑아 그 도구의 허용 목록과 맞댄다** — 두 벌이 될 수 없다.
     *
     * ⛔ **처방을 실행하지는 않는다.** 이 저장소의 처방 중에는 **모델을 부르는 것**이 있다
     * (`new --from` · `extract --write`). 「재려고 돌렸더니 돈이 나갔다」가 R145 의 그 사건이다.
     * ⇒ 여기서 재는 것은 **「거절당하지 않는가」**까지다. 그 위는 관문들이 실제로 돌며 잰다.
     */
    const allowedOf = (file) => {
      const at = join(ROOT, file);
      if (!existsSync(at)) { return undefined; }
      const m = /rejectUnknownFlags\([^,]+,\s*\[([^\]]*)\]/.exec(readFileSync(at, 'utf8'));
      /* `null` = 그 도구는 플래그를 안 거른다(잴 것이 없다). `undefined` = 파일이 없다. */
      return m ? new Set([...m[1].matchAll(/'([^']+)'/g)].map((x) => x[1])) : null;
    };
    const manifestTargets = Object.fromEntries(
      [...manifest.matchAll(/^\s{2}'?([a-z-]+)'?:\s*'([^']+)'/gm)].map((m) => [m[1], m[2]]),
    );
    ...
    for (const file of tools.filter((f) => f !== REGISTRY)) {
      ...
      const text = await readFile(join(ROOT, file), 'utf8');
      const code = text.split('\n').filter((l) => !/^\s*(\*|\/\*|\/\/)/.test(l)).join('\n');
      /* 두 모양 다 본다: 사람이 치는 `universe <명령> --f` 와 문서가 적는 `node <경로> --f`. */
      const forms = [
        [/universe ([a-z-]{2,})((?: --?[a-z-]+)+)/g, (name) => manifestTargets[name]],
        [/node ((?:observatory|bin|lib|fixtures|bigbang|beacon)\/[\w./-]+\.mjs)((?: --?[a-z-]+)*)/g, (name) => name],
      ];
      for (const [pattern, resolve] of forms) {
        for (const m of code.matchAll(pattern)) {
          const target = resolve(m[1]);
          if (!target) { continue; }
          const allowed = allowedOf(target);
          if (allowed === undefined) {
            dead.push(`${target} ← ${file} (그런 파일이 없다)`);
            continue;
          }
          if (allowed === null) { continue; }
          for (const flag of (m[2] ?? '').trim().split(/\s+/).filter(Boolean)) {
            flagged += 1;
            if (!allowed.has(flag) && !['--help', '--all'].includes(flag)) {
              dead.push(`${m[1]} ${flag} ← ${file}`);
            }
          }
        }
      }
    }
    /* 분모 가드 — 플래그가 붙은 처방을 하나도 못 봤으면 이 검사는 죽은 것이다(§8). */
    assert.ok(flagged > 10, `플래그 붙은 처방을 ${flagged}개만 봤다 — 훑개가 고장 났다(§8)`);
    assert.deepEqual(dead, [],
      '도구가 적은 처방이 **그대로 치면 거절당한다**:\n'
      + dead.map((d) => `   ${d}`).join('\n')
      + '\n   ⛔ 이름만 맞은 처방이다 — 사람은 「⛔ 모르는 플래그」를 보고 도구를 안 믿게 된다.');
    console.log(`✅ 플래그 붙은 처방 ${flagged}개가 전부 그 도구가 받는 것이다`);
  }
}

/**
 * **손 변이 틀이 자기가 만든 갈래를 전부 재는가** — ⛔ **다른 층에서** 잰다.
 *
 * ⚠️ 실측: 틀 안에 「만들어 놓고 안 재는 갈래」 가드를 넣어 뒀는데, **그 가드를 꺼 봐도
 * 아무도 안 물었다**(틀 자신도, 부품 시험도, 변이 관문도). 같은 층에서 재면 그 층이 무보호다 —
 * 옆 저장소 세션이 자기 틀에서 같은 한계를 실측하고 「없앨 수 있는 한계가 아니라 적어 두는
 * 한계다」라고 적었다.
 *
 * ⇒ **한 층 밖으로 옮긴다.** 여기서 틀을 **돌려서** 화면에 나온 판정과 소스의 `VERDICTS` 를 맞댄다.
 *   질문은 **소스**(무엇을 만들었나) · 답은 **실행 결과**(무엇을 쟀나) — 두 자리가 독립이다.
 *   ⛔ 둘 다 소스에서 뽑으면 대조가 아니라 **순환**이다(옆 세션이 세운 규칙).
 *
 * ⚠️ 이 검사도 하한이다: **이 검사를 지우면** 다시 무보호다. 다만 그 자리는 `verify-checks` 가
 * 변이로 문다 — 회귀가 **한 칸 더 밖에서** 끝난다. 무한히 밀어내지는 못한다(§8).
 */
{
  /* ⛔ **import 하지 않는다** — 그 파일은 스크립트라 불러들이면 **CLI 가 돌아** 쓰임을 찍고 죽는다
     (실측으로 겪었다). 소스에서 뽑는다. 질문은 소스, 답은 **실행 결과**라 여전히 독립이다. */
  const mutateSrc = await readFile(join(ROOT, 'observatory/mutate.mjs'), 'utf8');
  const block = /export const VERDICTS = \{([\s\S]*?)\};/.exec(mutateSrc)?.[1] ?? '';
  const made = [...block.matchAll(/'([^']+)'/g)].map((m) => m[1]);
  assert.ok(made.length >= 5, `판정 갈래를 ${made.length}개로 읽었다 — 훑개가 고장 났다(§8)`);

  const shown = await new Promise((resolve) => {
    const child = spawn(process.execPath, [join(ROOT, 'observatory/mutate.mjs'), '--self-test'],
  ...
  /* 분모 가드 — 틀이 아예 안 돌았으면 이 검사는 죽은 것이다. */
  assert.ok(shown.includes('손 변이 틀'), '손 변이 틀이 안 돌았다 — 이 검사는 죽은 것이다(§8)');

  const unmeasured = made.filter((v) => !shown.includes(`→ ${v}`));
  assert.deepEqual(unmeasured, [],
    `손 변이 틀이 **만들어 놓고 안 재는 갈래**가 있다: ${unmeasured.join(' · ')}\n`
    + '   ⛔ 갈래를 늘렸으면 자기 시험도 늘려라 — 안 재는 갈래는 있는지 없는지 아무도 모른다(§8).\n'
    + '   ⚠️ 틀 **안**의 같은 가드는 꺼 봐도 아무도 안 물었다. 그래서 이 검사가 밖에 있다.');
  console.log(`✅ 손 변이 틀이 만든 판정 ${made.length}가지를 자기 시험이 전부 낸다`);
}

/**
 * **관문이 자기가 못 재는 것을 적었는가** (§8).
 *
 * ⚠️ 실측: 이 세션에 만든 탐침·도구 일곱 중 **넷이 자기 한계를 안 적었다.**
 * 그리고 그중 하나(`loop-state`)가 나르는 계약은 **스스로 그렇게 말하고 있었다**:
 *   「판단의 내용이 맞는지는 재지 않는다 — `accepted` 사유의 **길이는 보지만 타당성은 못 본다**」
 * ⇒ **계약은 한계를 말하는데 그것을 나르는 도구가 안 날랐다.** 그러면 화면을 보는 사람은
 *   그 도구가 그것까지 잰다고 믿는다 — 「재는 척」이 되는 자리다.
 *
 * ⛔ **못 재는 것을 적는 것은 습관이었다.** 습관은 다음 회전의 내가 빼먹는다 ⇒ 검사로 바꾼다.
 * ⚠️ 이 검사도 **하한이다**: 「적었는가」만 보지 **적은 것이 맞는가**는 못 본다.
 *   ⛔ 그래서 이 검사가 초록이어도 「한계가 다 적혔다」는 뜻이 아니다.
 */
{
  const owned = (await new Promise((resolve) => {
    const child = spawn('git', ['ls-files', 'observatory'], { cwd: ROOT, stdio: ['ignore', 'pipe', 'ignore'] });
    let out = '';
    child.stdout.on('data', (d) => { out += d; });
    child.on('close', (code) => resolve(code === 0 ? out.split('\n').filter(Boolean) : null));
    child.on('error', () => resolve(null));
  ...
  if (owned === null) {
    console.log('⏭  못 재는 것을 적었는가 — git 에게 못 물었다(배달본이다). 못 쟀다.');
  } else {
    /* ⛔ **판정하는 도구만 본다.** 생성기(`render-*`)는 「무엇을 못 재는가」를 말할 자리가 아니다. */
    const judges = owned.filter((f) => /^observatory\/(verify|probe|loop-state|repeat|mutate)/.test(f)
      && /\.(mjs|sh)$/.test(f));
    assert.ok(judges.length > 10, `판정 도구를 ${judges.length}개만 봤다 — 훑개가 고장 났다(§8)`);
    ...
    for (const file of judges) {
      ...
      const text = await readFile(join(ROOT, file), 'utf8');
      /**
       * ⛔ **문구를 하나로 강제하지 않는다**(§9). 실측: 기존 도구들이 각자 다른 말로 적고 있었다 —
       * 「못 본다」·「못 잡는다」·「못 셌다」·「못 돌았다」·「안 잰다」. 하나를 고르면 나머지가
       * **적었는데도 안 적은 것으로** 잡힌다(첫 판에 11개가 그렇게 잡혔다).
       * ⇒ **모양이 아니라 성질**로 본다: 「못/안 + …」을 §8 이나 ⚠️/⛔ 와 **같은 문단에서** 말하는가.
       */
      const limits = /(못|안)\s?[가-힣]{1,5}(는다|다|았다|는)[^\n]{0,60}/g;
      const admits = [...text.matchAll(limits)].some((m) => {
        const at = m.index ?? 0;
        const around = text.slice(Math.max(0, at - 200), at + 200);
        return /§8|⚠️|⛔/.test(around);
      });
      if (!admits) {
        silent.push(file);
      }
    ...
    assert.deepEqual(silent, [],
      `**자기가 못 재는 것을 안 적은 판정 도구**가 있다: ${silent.join(' · ')}\n`
      + '   ⛔ 안 적으면 사람은 그 도구가 **그것까지 잰다고 믿는다** — 「재는 척」이 되는 자리다.\n'
      + '   ⚠️ 실측: 계약은 「판단의 타당성은 못 본다」고 스스로 말하는데, 그걸 나르는 도구가\n'
      + '      그 말을 안 날라서 화면에서 사라졌다.');
    console.log(`✅ 판정 도구 ${judges.length}개가 전부 자기 한계를 적었다 (⚠️ 적은 것이 맞는지는 못 잰다)`);
  }
}

/**
 * **구조 설명 카드의 자리와 id** (`lib/blueprint.mjs`).
 *
 * ⛔⛔ `unitId` 는 **그대로 파일 이름이 된다.** 구분자나 `..` 가 살아 있으면
 * 지식창고 **밖의 파일**을 건드린다 — 엔진의 `isSafeCardId` 가 같은 값을 치른 자리다(R76 계열).
 * ⚠️ 이 둘은 자식이 만들고 **시험을 안 붙였다** — 부품 시험이 「깨져도 모른다」고 물어서 넣는다.
 */
{
  const { blueprintPaths, unitId, isRouteTable, ROUTE_MIN } = await import(
    pathToFileURL(join(ROOT, 'lib/blueprint.mjs')).href);

  const at = blueprintPaths('/tmp/은하');
  assert.ok(at.dir.endsWith('/universe/blueprint'), `카드는 은하의 universe/ 아래에 산다: ${at.dir}`);
  assert.ok(at.index.endsWith('/universe/BLUEPRINT.md'), `목차도 같은 자리다: ${at.index}`);

  /* ⛔ **경로를 탈출하지 못한다** — 이 한 줄이 이 부품의 존재 이유다. */
  for (const bad of ['../../etc/passwd', '/etc/passwd', './src/pages', '..', '', null]) {
    const id = unitId(bad);
    assert.ok(!id.includes('/') && !id.includes('\\'),
      `자리 id 에 구분자가 남았다 — 파일이 창고 밖에 생긴다: ${bad} → ${id}`);
    assert.ok(!/(^|[^\w])\.\.($|[^\w])/.test(id), `자리 id 에 \`..\` 가 남았다: ${bad} → ${id}`);
    assert.notEqual(id, '', `빈 id 는 파일 이름이 될 수 없다: ${bad}`);
  }
  assert.equal(unitId('./src/pages'), 'src-pages', '평범한 자리는 읽을 수 있는 id 가 된다');

  /* ⚠️ 「라우트 표인가」는 **하한**이다 — 수가 적으면 표가 아니라고 본다. 그 문턱이 살아 있는지만 잰다. */
  assert.equal(isRouteTable(Array(ROUTE_MIN).fill('/x')), true, `${ROUTE_MIN}개면 라우트 표로 본다`);
  assert.equal(isRouteTable(Array(ROUTE_MIN - 1).fill('/x')), false, '문턱 아래는 표가 아니다');
  assert.equal(isRouteTable(null), false, '없는 것은 표가 아니다 — 터지지 않는다');
  console.log('✅ 구조 카드의 자리와 id (경로를 탈출하지 못한다)');
}

/**
 * **구조 설명 카드 — 훑개 · 예산 · 1차 검사 · 렌더** (`lib/blueprint.mjs`).
 *
 * ⛔⛔ 여기 부품은 전부 **조용해지는 쪽으로 틀린다.** 훑개가 눈멀면 「자리 0개」, 예산이 잘린
 * 수를 안 말하면 「전부 봤다」, 1차가 표식을 놓치면 **초안이 완성본 행세**를 한다 —
 * 셋 다 화면은 정상으로 보인다(§8 이 하루에 다섯 번 잡은 그 모양).
 * ⇒ 그래서 재는 축은 하나다: **못 본 것을 못 봤다고 말하는가.**
 * ⚠️ 「같은 값인가」는 안 잰다 — 그건 시험이 아니라 **복사본**이라 같이 낡는다.
 */
{
  const {
    ENTRY_FILE, routeLiterals, collectUnits, budgetSlice, REQUIRED_FIELDS,
    PLACEHOLDER, cardProblems, renderCard, renderIndex, BLUEPRINT_PROMPT, foldUnit,
  } = await import(pathToFileURL(join(ROOT, 'lib/blueprint.mjs')).href);

  /* ── 예산: **잘랐으면 잘랐다고 말해야 한다**(§8) ─────────────────────────
     ⛔ 조용히 자르면 「전부 봤다」와 구별이 안 된다 — 모델에 안 보낸 파일이 그렇게 사라진다. */
  for (const [items, max, wantTaken, wantSkipped] of [
    [[1, 2, 3, 4, 5], 2, 2, 3],
    [[1, 2], 5, 2, 0],
    [[], 3, 0, 0],
  ]) {
    const cut = budgetSlice(items, max);
    assert.equal(cut.taken.length, wantTaken, `${items.length}개를 ${max}개로 잘랐는데 남은 수가 다르다`);
    assert.equal(cut.skipped, wantSkipped, '**안 본 수**가 틀리다 — 이 수가 곧 화면에 찍히는 「안 봤다」다');
    /* ⛔ **분모 보존.** 본 것 + 안 본 것이 원래 수와 다르면 어느 쪽인지 아무도 모른다. */
    assert.equal(cut.taken.length + cut.skipped, items.length, '자르고 나서 수가 안 맞는다 — 파일이 조용히 증발한다');
  }
  /* ⛔ 상한을 안 주거나 0·음수를 주면 **전부 본다.** 「0개만 본다」로 읽으면 조용히 전멸한다. */
  for (const bad of [undefined, 0, -1, NaN]) {
    const cut = budgetSlice([1, 2, 3], bad);
    assert.equal(cut.taken.length, 3, `상한이 ${String(bad)} 일 때 아무것도 안 본다 — 조용한 전멸이다`);
    assert.equal(cut.skipped, 0, '안 잘랐는데 잘랐다고 말한다');
  }
  assert.deepEqual(budgetSlice(null, 2), { taken: [], skipped: 0 }, '없는 목록에서 터진다');

  /* ── 훑개: **못 찾은 0** 과 **없는 0** 을 섞지 않는다 ────────────────────── */
  assert.deepEqual(
    routeLiterals("const a = '/'; const b = \"/orders\"; const c = `/orders/:id`;"),
    ['/', '/orders', '/orders/:id'],
    'URL 모양 리터럴을 따옴표 세 종류에서 다 못 읽는다');
  /* ⛔⛔ 이 한 줄이 훑개의 존재 이유다 — 상대 임포트와 URL 을 라우트로 세면 **모든 파일이
     라우터**가 되고, 그러면 자리 수가 부풀어 「전부 설명해야 한다」가 된다. */
  assert.deepEqual(routeLiterals("import x from './rel'; const u = 'https://a/b'; import y from '../up';"), [],
    '상대 임포트나 URL 을 라우트로 셌다 — 아무 파일이나 라우터가 된다');
  assert.deepEqual(routeLiterals("['/a', '/a']"), ['/a'], '같은 경로를 두 번 센다');
  for (const empty of ['', null, undefined]) {
    assert.deepEqual(routeLiterals(empty), [], `없는 본문에서 터진다: ${String(empty)}`);
  }

  /* ⛔ `index` 는 **번들러가 정한 이름**이라 열거해도 안 낡는다(§9 의 예외).
     그런데 **접두사만 같은 것**까지 삼키면 아무 파일이나 「들어오는 자리」가 된다. */
  assert.equal(ENTRY_FILE.test('src/x/index.tsx'), true, '입구 파일을 못 알아본다');
  assert.equal(ENTRY_FILE.test('index.ts'), true, '뿌리의 입구를 못 알아본다');
  assert.equal(ENTRY_FILE.test('src/indexing.ts'), false, '`indexing.ts` 를 입구로 삼켰다 — 아무 폴더나 단위가 된다');
  assert.equal(ENTRY_FILE.test('src/x/main.tsx'), false, '입구가 아닌 것을 입구로 본다');

  /* ── 자리 모으기: **좌표가 이기고 · 문턱 아래는 안 세고 · 이름에 안 기댄다**(§9) ── */
  const scanned = [
    { at: 'src/화면묶음/index.tsx', routes: [] },
    { at: 'src/화면묶음/Detail.tsx', routes: [] },
    { at: 'src/길잡이.tsx', routes: ['/', '/orders', '/orders/:id'] },
    { at: 'src/한칸.ts', routes: ['/'] },
    { at: 'src/indexing.ts', routes: [] },
  ];
  const units = collectUnits({ solarSystems: [{ name: '가게', srcDir: 'src/화면묶음' }] }, scanned);
  const whereBy = Object.fromEntries(units.map((u) => [u.at, u]));
  /* ⚠️ 자리 이름이 **한글**이다 — 폴더 이름을 열거하는 훑개였다면 여기서 0개가 된다(§9). */
  assert.equal(units.length, 2, `한글 이름의 자리를 못 찾았다(${units.length}개) — 훑개가 이름에 기대고 있다`);
  assert.equal(whereBy['src/화면묶음']?.kind, '좌표',
    '좌표가 선언한 자리를 `index.*` 가 덮어썼다 — 그 팀이 적어 둔 것이 이겨야 한다');
  assert.equal(whereBy['src/길잡이.tsx']?.routes.length, 3, '라우터 단위가 자기 경로를 안 들고 있다');
  assert.ok(!whereBy['src/한칸.ts'], 'URL 하나짜리를 라우터로 셌다 — 문턱이 죽었다');
  assert.ok(!whereBy['src/indexing.ts'], '`indexing.ts` 로 단위를 만들었다');
  assert.equal(units.length, new Set(units.map((u) => u.at)).size, '같은 자리를 두 번 세운다');
  /* ⛔ **빈손은 터지지 않고 빈손으로 온다** — 「0개」를 「없다」로 바꾸는 것은 부르는 쪽의 일이고,
     부르는 쪽은 그것을 ⚪ 로 말한다(`observatory/blueprint.mjs`). 여기서 터지면 그 갈래가 안 온다. */
  assert.deepEqual(collectUnits({}, []), [], '빈 저장소에서 터진다');
  assert.deepEqual(collectUnits(null, null), [], '아무것도 없을 때 터진다');

  /* ── 1차 검사: **초안을 완성본으로 세지 않는다** ────────────────────────── */
  const BODY = '## 이 자리는 무엇인가\n주문 화면 묶음이다. 목록과 자세히 보기 두 화면이 여기 살고, 결제 화면은 옆 자리에 있다.\n\n## 화면·메뉴\n목록 · 자세히 보기.';
  assert.deepEqual(cardProblems({ file: 'c.md', front: { unit: 'u', at: 'src', kind: 'index' }, body: BODY, atExists: true }),
    [], '멀쩡한 카드를 어긋남이라 한다');
  /* ⛔ 필수 칸은 **명부에 적기만 하면 안 된다** — 하나씩 비워서 정말 보는지 잰다. */
  assert.ok(REQUIRED_FIELDS.length >= 3, `필수 칸이 ${REQUIRED_FIELDS.length}개뿐이다 — 훑개가 아니라 명부가 비었다(§8)`);
  for (const key of REQUIRED_FIELDS) {
    const front = { unit: 'u', at: 'src', kind: 'index' };
    delete front[key];
    assert.ok(cardProblems({ file: 'c.md', front, body: BODY, atExists: true }).some((x) => x.includes(key)),
      `필수 칸 \`${key}\` 를 비웠는데 아무 말이 없다 — 명부에 이름만 적고 안 보고 있다`);
  }
  assert.ok(cardProblems({ file: 'c.md', front: { unit: 'u', at: 'src/없다', kind: 'index' }, body: BODY, atExists: false })
    .some((x) => x.includes('자리가 저장소에 없다')),
  '**없는 자리를 가리키는 설명**을 통과시켰다 — 사람이 그 자리를 찾으러 간다');
  /* ⛔ 「빈 카드」가 통과하면 그 자리는 **덮인 것으로 세어져 조용해진다** — 0 이 무죄가 되는 자리다. */
  assert.ok(cardProblems({ file: 'c.md', front: { unit: 'u', at: 'src', kind: 'index' }, body: '## 없음\n\n', atExists: true })
    .some((x) => x.includes('비어 있다')),
  '빈 카드를 통과시켰다 — 그 자리는 「설명이 있다」로 세어지고 영영 조용해진다');

  /* ── 렌더: **사람이 채울 자리를 지우지 않는다** ─────────────────────────── */
  const unit = { id: 'src-pages', at: 'src/pages', kind: 'index', why: '입구가 있다' };
  const card = renderCard({
    unit, files: [{ at: 'src/pages/index.tsx' }], skipped: 3,
    body: BODY, measuredAt: '2026-01-01T00:00:00.000Z', galaxyName: '은하',
  });
  const front = frontmatter(card);
  for (const key of REQUIRED_FIELDS) {
    assert.notEqual(String(front[key] ?? ''), '', `렌더가 필수 칸 \`${key}\` 를 안 싣는다 — 자기가 만든 카드가 자기 검사에 걸린다`);
  }
  assert.match(card, /안 본 파일 3개/, '카드가 **안 본 파일 수**를 안 들고 있다 — 화면은 지나가고 파일은 남는다(§8)');
  /* ⛔ **한 바퀴 돈다**: 렌더가 만든 카드가 1차 검사를 그대로 통과해야 한다. 안 그러면
     `--write` 를 돌린 사람이 **자기가 만든 것에** 매번 빨간불을 본다. */
  assert.deepEqual(
    cardProblems({ file: 'x.md', front, body: card.replace(/^---\n[\s\S]*?\n---\n/, ''), atExists: true }),
    [], '렌더가 만든 카드가 **자기 1차 검사에 걸린다**');
  /* ⛔⛔ 초안 표식은 **살아남아야 한다.** 렌더가 지우면 안 채운 카드가 완성본 행세를 하고,
     1차는 물 것이 없어져 조용해진다 — 표식이 두 자리에서 갈리면 정확히 그 일이 난다. */
  const draft = renderCard({
    unit, files: [], skipped: 0,
    body: `${BODY}\n${PLACEHOLDER} 여기는 아직 안 적었다`, measuredAt: '2026-01-01T00:00:00.000Z', galaxyName: '은하',
  });
  assert.ok(draft.includes(PLACEHOLDER), '렌더가 초안 표식을 지웠다 — 안 채운 카드가 완성본 행세를 한다');
  assert.ok(cardProblems({ file: 'd.md', front: frontmatter(draft), body: draft, atExists: true })
    .some((x) => x.includes('사람이 채울 자리')),
  '초안 표식이 든 카드를 1차가 그냥 통과시켰다 — 1차가 장식이다');

  /* ⛔ 모델에게 주는 말과 검사가 무는 표식이 **갈리면** 모델이 매번 물리는 카드를 쓴다.
     ⚠️ 값을 그대로 베끼지 않는다 — **두 자리가 같은 표식을 쓰는가**만 잰다. */
  assert.ok(BLUEPRINT_PROMPT.includes(PLACEHOLDER),
    '프롬프트가 1차 검사와 **다른 표식**을 말한다 — 모델이 매번 물리는 카드를 쓰게 된다');
  assert.ok(BLUEPRINT_PROMPT.includes('프론트매터'),
    '프롬프트가 프론트매터를 금하지 않는다 — 카드에 `---` 가 두 겹이 되어 파서가 엉뚱한 칸을 읽는다');
  assert.ok(/지어내지 마라/.test(BLUEPRINT_PROMPT),
    '프롬프트가 **경로를 지어내지 마라**를 안 말한다 — 없는 자리를 가리키는 카드가 나온다');

  /* ── 모델 입력: **안 준 것을 안 줬다고 적는가** ─────────────────────────── */
  const folded = foldUnit({ unit, samples: [{ at: 'src/pages/index.tsx', text: 'export const A = 1;' }], skipped: 4 });
  assert.match(folded, /4개는 안 줬다/,
    '**안 준 파일 수**가 입력에 없다 — 모델은 준 것이 전부인 줄 알고 「이 자리엔 그것뿐」이라 쓴다');
  assert.ok(folded.includes('src/pages/index.tsx'), '파일 경로가 입력에 없다 — 모델이 경로를 지어내게 된다');
  assert.ok(folded.includes(unit.at), '어느 자리를 설명하는지가 입력에 없다');
  assert.ok(foldUnit({ unit, samples: [], skipped: 0 }).includes('없음'),
    'URL 이 없는 자리를 조용히 넘긴다 — 「없다」와 「안 봤다」가 섞인다');

  /* ── 명부: **안 한 자리 수**가 파일에 남는가 ────────────────────────────── */
  const idx = renderIndex({
    galaxyName: '은하', rows: ['| src/pages | index | [a.md](blueprint/a.md) |'],
    command: 'node observatory/blueprint.mjs --galaxy 은하 --write', skippedUnits: 2,
  });
  assert.match(idx, /\*\*설명하지 않은 자리 2개\*\*/,
    '명부가 **안 한 자리 수**를 안 들고 있다 — 목차만 보면 「전부 있다」로 보인다(§8)');
  assert.ok(idx.includes('blueprint/a.md'), '명부가 카드를 안 가리킨다');
  assert.ok(idx.includes('--write'), '다시 뽑는 명령이 없다 — 카드가 어디서 왔는지 아무도 모른다');
  assert.ok(!renderIndex({ galaxyName: '은하', rows: [], command: 'x', skippedUnits: 0 }).includes('설명하지 않은 자리'),
    '안 한 자리가 0인데도 경고가 남는다 — 늑대소년이 된다');

  console.log('✅ 구조 카드 부품 — 잘린 수를 말하고, 초안 표식을 안 지우고, 이름에 안 기댄다');
}
...
/* ── 허용하지만 **읽는 코드가 없는** 플래그 (R72).
   ...
   **읽는 방식**으로 본다. */
/* ⚠️ `.includes('--x')` 는 **어느 변수든** 진짜 읽기다. 예전엔 `argv.includes` 만 인정했는데,
   그것이 나를 **틀린 구현으로 밀었다**(실측): `--` 뒤 명령을 뺀 `mine.includes(...)` 로 읽는 게
   옳은데 검사를 통과시키려고 `argv.includes(...)` 로 바꿨고, 그러자 **`--` 뒤에 있는 `--self-test`
   까지 읽어** 변이를 걸어 놓고 자기 시험이 대신 돌았다. ⛔ 검사가 구현을 왜곡하면 그 검사가 잘못이다.
   ⚠️ 그래도 **문자열이 있는지**로는 안 본다 — `['git','status','--porcelain']` 같은 인자에 속는다.
   `.includes(` 라는 **호출 형태**를 요구하는 것이 그 둘 사이의 자리다. */
/**
 * ⚠️ **읽는 방식**으로 본다 — 문자열이 있는지로 보면 `git status --porcelain` 의 인자에 속는다.
 * ⛔ 그렇다고 **함수 이름을 열거하면**(§9) 같은 일을 하는 다른 이름이 「안 읽는다」로 잡힌다:
 *   실측(R163): `flagValue('--limit')` · `number('--max-files', 12)` 가 값을 **읽고 있는데**
 *   훑개가 `flag(` 만 알아서 **두 도구를 거짓으로 물었다.**
 * ⇒ **`(…'--x'` 꼴의 호출**이면 읽기로 본다. 이름은 안 본다 — 괄호가 있어야 하는 것은 그대로다.
 */
const READS = [".includes('%s')", "('%s')", "('%s',", "argv.indexOf('%s')"];
...
if (isUniverseSource) {
  ...
  /**
   * ⛔⛔ **같은 도구에 사유를 두 번 적으면 뒤엣것이 앞엣것을 조용히 덮는다.**
   *
   * ⚠️ 실측(2026-09-09): `observatory/loop-state.mjs` 가 `NOT_GATED` 에 **두 번** 적혀
   * 있었다. 앞엣것은 정직했고(「지금 아무도 안 잰다 — 재던 탐침이 TC 도구와 함께 나갔다」),
   * 뒤엣것은 **낡아서 거짓**이었다(「탐침이 세 갈래를 잰다」 — 그 탐침은 이 가지에 없다).
   * 객체 리터럴이라 **거짓인 쪽이 살아남았고**, 바로 위 주석 블록은 진실을 길게 적어 두고도
   * 코드가 그 반대를 말하고 있었다.
   * ⇒ 「판단하지 않은 도구 0개」를 세는 장치에서 **판단이 뒤바뀌어도 아무도 몰랐다.**
   *
   * 소스에 적힌 키의 수와 객체가 든 키의 수를 견준다 — 다르면 덮인 것이 있다.
   */
  const gatesSrc = await readFile(join(ROOT, 'lib/gates.mjs'), 'utf8');
  const notGatedBlock = gatesSrc.slice(gatesSrc.indexOf('export const NOT_GATED'));
  const writtenKeys = [...notGatedBlock.matchAll(/^\s{2}'([^']+)':/gm)].map((m) => m[1]);
  const dupes = writtenKeys.filter((k, i) => writtenKeys.indexOf(k) !== i);
  assert.ok(writtenKeys.length > 0, 'NOT_GATED 의 키를 하나도 못 읽었다 — 훑개가 고장 났다(§8)');
  assert.deepEqual([...new Set(dupes)], [],
    `NOT_GATED 에 같은 도구가 두 번 적혀 있다 — 뒤엣것이 앞엣것을 조용히 덮는다: ${[...new Set(dupes)].join(' · ')}`);
  console.log(`✅ 관측소 도구 ${tools.length}개가 전부 관문이거나 사유가 있다 (밖 ${Object.keys(NOT_GATED).length}개 · 중복 0)`);
}
...
/* ── 커밋 관문의 **안내가 훅과 같은 것을 세는가** (실측 2026-09-08).
   ⛔ `universe hooks --install` 은 무엇이 도는지 **훅에서 읽어서** 말한다 — 손 목록이 낡는 것을
   막으려고 R73 에 그렇게 바꿨다. 그런데 훑개가 `^run '` 이라 **줄 앞에 여백이 있는 호출을
   못 봤고**, 하필 `법칙 관측`(observe)만 `if/else` 안에 들여쓰여 있었다.
   ⇒ 오늘 훅에 들어간 **가장 중요한 검사**가 안내에서만 통째로 사라졌다. 훅은 돌고 있었으니
   초록불로는 영영 안 보인다 — 「말한 것」과 「도는 것」이 갈린 자리다(§8).
   ⚠️ 세는 법을 **일부러 다르게** 썼다 — 여기는 정규식이 아니라 「줄의 첫 낱말이 `run` 인가」다.
   같은 정규식으로 세면 훑개가 틀려도 시험이 같이 틀린다. */
if (existsSync(join(ROOT, '.githooks/pre-commit'))) {
  const hookText = await readFile(join(ROOT, '.githooks/pre-commit'), 'utf8');
  const byFirstWord = hookText.split('\n')
    .map((line) => line.trim())
    .filter((line) => line.split(/\s+/)[0] === 'run')
    .map((line) => (line.match(/'([^']+)'/) ?? [])[1])
    .filter(Boolean);
  const { hookLabels } = await import(pathToFileURL(join(ROOT, 'lib/gates.mjs')).href);
  const announced = hookLabels(hookText);
  assert.deepEqual(announced, byFirstWord,
    `커밋 관문의 안내가 훅과 다른 것을 센다 — 안내 ${announced.length}개(${announced.join(' · ')}) `
    + `vs 훅 ${byFirstWord.length}개(${byFirstWord.join(' · ')}). `
    + '`lib/gates.mjs` 의 `hookLabels` 가 훅의 호출을 하나라도 놓치면 사람은 **안 도는 줄 안다**.');
  assert.ok(announced.includes('법칙 관측'),
    '커밋 관문에 `법칙 관측`(observe)이 없다 — 관문이 「우주가 멀쩡한가」만 보고 '
    + '**「당신 코드가 나빠졌는가」는 아무도 안 본다.** 위반이 늘어난 커밋이 그냥 지나간다(실측 2026-09-08).');
  console.log(`✅ 커밋 관문의 안내가 훅과 같은 ${announced.length}개를 센다 (법칙 관측 포함)`);
}
...
/**
 * ── **「끝났는가」의 판정 계약**(`lib/loop-contract.mjs`).
 *
 * ⛔ 이 계약은 2026-09-09 에 `qa/dist/run/cli.js` 에서 **옮겨 온 것**이다. 옮기면서 뜻이
 * 바뀌면 「끝났다」의 정의가 조용히 갈린다 — 그래서 **경계마다** 잰다.
 * 옮기기 전, 원본과 여섯 갈래를 나란히 돌려 답이 같은 것을 확인했다(종료코드·판정·목록·통계).
 */
{
  const {
    NON_VERIFYING_ORIGINS, MIN_ACCEPTED_WHY, validateVerdict, measurabilityOf,
    buildRunReport, evaluateDone, caseIdOf, statusOf, fromPlaywrightJson, judgeRun,
  } = await import(pathToFileURL(join(ROOT, 'lib/loop-contract.mjs')).href);

  /* ⛔ **출처를 모르는 TC 는 검증이 아니다.** 이 둘이 빠지면 자기 채점이 열린다. */
  assert.ok(NON_VERIFYING_ORIGINS.includes('unknown') && NON_VERIFYING_ORIGINS.includes('derived-from-code'),
    '검증에서 빼는 출처가 줄었다 — 출처를 모르는 TC 로 「끝났다」가 나온다');

  /* ⛔ **사유 없는 accepted 는 판단이 아니라 치운 것이다.** */
  assert.equal(validateVerdict(null).valid, false, '판정 없음을 판단으로 셌다');
  assert.equal(validateVerdict({ kind: 'accepted', why: '일단' }).valid, false, '자리표시자 사유를 판단으로 셌다');
  assert.equal(validateVerdict({ kind: 'accepted', why: 'ㄱ'.repeat(MIN_ACCEPTED_WHY - 1) }).valid, false,
    `${MIN_ACCEPTED_WHY}자 미만 accepted 를 판단으로 셌다`);
  assert.equal(validateVerdict({ kind: 'fixed', why: '셀렉터를 고쳤다' }).valid, true, 'fixed 를 판단이 아니라고 했다');

  /* ⛔ **전제 0개는 「다 섰다」가 아니라 「못 쟀다」다.** */
  assert.equal(measurabilityOf([]).measurable, false, '전제 0개를 「다 섰다」로 접었다');
  const folded = buildRunReport({
    preconditions: [{ id: 'session', ok: false, detail: '세션이 죽었다' }],
    cases: [{ id: 'TC-1', origin: 'human', status: 'failed', attribution: 'star', verdict: null }],
  });
  assert.equal(folded.cases[0].status, 'unmeasured', '전제가 안 섰는데 케이스를 fail 로 셌다');
  assert.equal(evaluateDone(folded).exitCode, 3, '전제가 안 선 주행을 ❌ 로 셌다');

  /* ⛔ **flaky 는 통과가 아니다** — 같은 조건에서 갈리는 관측은 아무것도 증명하지 않는다. */
  assert.equal(statusOf('flaky').status, 'unmeasured', 'flaky 를 통과로 접었다');
  assert.equal(statusOf('skipped').status, 'unmeasured', '비켜선 것을 통과로 접었다');

  /* 케이스 id — 제목의 `TC-…` 를 쓰고, 없으면 파일+제목으로 유일하게 만든다. */
  assert.equal(caseIdOf({ title: 'TC-100 대시보드' }), 'TC-100', 'TC id 를 못 집었다');
  assert.match(caseIdOf({ title: '제목만', file: 'a.spec.ts' }), /a\.spec\.ts/, '파일로 id 를 안 만들었다');

  /* ⛔ **생 Playwright 리포트는 출처를 모른다** → 검증 분모 0 → 못 쟀다(3). */
  const pw = {
    config: {}, errors: [],
    suites: [{ file: 'e2e/a.spec.ts', specs: [
      { title: 'TC-1 산다', tests: [{ status: 'expected' }] },
      { title: 'TC-2 깨졌다', tests: [{ status: 'unexpected' }] },
    ] }],
  };
  assert.equal(fromPlaywrightJson(pw).cases[0].origin, 'unknown', '출처를 모르는데 안다고 했다');
  assert.equal(judgeRun(pw).done.exitCode, 3, '출처를 모르는 TC 로 판정을 냈다');

  /* 명부를 주면 분모가 서고, 판정 없는 fail 이 남아 1 이다. */
  const origins = { 'TC-1': { origin: 'human' }, 'TC-2': { origin: 'human' } };
  assert.equal(judgeRun(pw, { origins }).done.exitCode, 1, '판단하지 않은 fail 을 「끝났다」로 셌다');

  /* 판정이 붙으면 fail 이 있어도 끝난 것이다 — 「fail 0」이 종료 조건이 아니다. */
  const judged = judgeRun({
    preconditions: [{ id: 'session', ok: true }],
    cases: [
      { id: 'TC-1', origin: 'human', status: 'passed', attribution: 'star', verdict: null },
      { id: 'TC-2', origin: 'human', status: 'failed', attribution: 'star', verdict: { kind: 'fixed', why: '고쳤다' } },
    ],
  });
  assert.equal(judged.done.exitCode, 0, 'fail 에 판정이 붙었는데 「안 끝났다」고 했다');
  console.log('✅ 「끝났는가」 계약 — 출처 없는 TC 는 ⚪ · flaky 는 통과가 아니다 · 종료 조건은 「판단하지 않은 fail 0」');
}

/**
 * ── **관문을 만들고 문서에 안 적으면 아무도 그게 있는 줄 모른다.**
 *
 * ⚠️ 실측(2026-09-09): 관문 34개 중 **7개가 사용법 문서에 이름조차 없었다** —
 * 탐침 다섯(`probe-draft`·`probe-clone`·`probe-repos`·`probe-blueprint`·`probe-loop`)과
 * 자체시험 둘. 문서의 「관문에 있는데 명령이 없는 것」 표는 **둘만** 적고 있었다.
 * ⛔ 이건 R53 과 같은 종류다 — 있는데 문서에 없으면 **없는 것과 같다.** 다만 이쪽은
 * 소비자가 아니라 **다음에 이 저장소를 만지는 사람**이 못 본다: 관문을 고치면서 무엇을
 * 깨뜨리는지 모른다.
 *
 * ⛔ **수를 세지 않는다** — 「몇 개」는 늘 때마다 낡는다. **하나하나 이름으로** 대조한다.
 * 짚는 방법은 둘 다 허용한다: 배분된 명령이면 `universe <이름>`, 아니면 **파일 경로**.
 */
{
  const { GATES } = await import(pathToFileURL(join(ROOT, 'lib/gates.mjs')).href);
  const usage = await readFile(join(ROOT, 'docs/02-usage.md'), 'utf8').catch(() => '');
  if (usage === '') {
    console.log('⏭  관문 문서화 검사 — 사용법 문서가 없다(배달본이다). 못 쟀다.');
  } else {
    ...
    const nameOf = new Map(
      [...routerBlock.matchAll(/^\s*'?([a-z][a-z-]*)'?:\s*'([^']+)'/gm)].map((m) => [m[2], m[1]]),
    );
    /* ⛔ 분모가 0이면 이 검사는 죽은 것이다(§8). */
    assert.ok(nameOf.size > 20, `배분표를 ${nameOf.size}개만 읽었다 — 훑개가 고장 났다`);
    const undocumented = GATES.filter((g) => {
      const name = nameOf.get(g.file);
      if (name && new RegExp(`universe\\s+${name}\\b`).test(usage)) {
        return false;
      }
      return !usage.includes(g.file);
    }).map((g) => `${g.label} (${g.file})`);
    assert.deepEqual(undocumented, [],
      `사용법 문서가 안 짚는 관문 — 있는 줄도 모른다: ${undocumented.join(' · ')}`);
    console.log(`✅ 관문 ${GATES.length}개가 전부 사용법 문서에 이름이 있다`);
  }
}
