...
if (isUniverseSource && existsSync(join(ROOT, '.githooks/pre-commit'))
...
{
  const { createOpenRouterAsk, OPENROUTER_DEFAULT_MODEL, OPENROUTER_ENV } =
    await import(pathToFileURL(join(ROOT, 'bigbang/agent-openrouter.mjs')).href);
  assert.equal(OPENROUTER_ENV, 'OPENROUTER_API_KEY');
  assert.match(OPENROUTER_DEFAULT_MODEL, /^anthropic\//, '구독 별칭(sonnet)은 이 레인에서 안 통한다 — 슬러그여야 한다');
  ...
  console.log('✅ OpenRouter 레인 — 이력을 나르고 · 비용을 안 지어내고 · 빈 답을 통과로 안 센다');
}
...
if (isUniverseSource) {
  ...
  assert.ok(!/\?\?\s*'openrouter'/.test(src), 'openrouter 가 기본값이 되는 갈래가 있다 — 사람이 안 골랐는데 과금된다');
  /* 레인 검증이 **별을 쓰기 전**이어야 한다 — 뒤면 「멈춘다」고 말해 놓고 별을 남긴다(실측으로 밟았다). */
  assert.ok(src.indexOf('openrouter 레인이 서지 않는다') < src.indexOf('별이 태어났다'),
    '레인 검증이 별을 쓴 뒤다 — 못 잴 곳에 별을 남긴다');
  console.log('✅ 레인은 자동으로 안 갈아탄다 (기본 구독 · 검증이 별보다 먼저)');
}
