...
describe('--json 계약 — 린터가 돌면 반드시 유효 JSON', () => {
  ...
  it('빌드된 dist 에서도 계약이 성립한다 — 소비 쪽이 실제로 부르는 것은 dist 다', () => {
    ...
    if (!existsSync(built)) {
      // ⛔ 조용히 넘어가지 않는다 — 안 잰 것이 초록으로 보이는 자리다.
      throw new Error(
        `dist 가 없어 이 축을 재지 못했다: ${built}\n` +
          '  `npm run build` 를 먼저 돌려야 잰다 (CI 는 test 앞에서 빌드한다).',
      );
    }
    const result = spawnSync(
      process.execPath,
      [built, '--format', 'sheet', '--json', 'tests/fixtures/handoff/spec-dirty.json'],
...
/**
 * 위 시험들은 `tsx` 로 **소스**를 부른다. 그런데 소비 쪽이 실제로 부르는 것은
 * **빌드된 `dist/lint/cli.js`** 다 — `node ../qa-harness/dist/lint/cli.js …`.
 *
 * 소스에서 계약이 성립해도 빌드 설정이 어긋나면(출력 경로·확장자·shebang)
 * 소비 쪽에서만 깨진다. 그래서 **실제로 부르는 것을 재는** 축을 따로 둔다.
 *
 * ⚠️ **dist 가 없으면 건너뛰지 않고 만든다.** 조용한 skip 은 이 저장소가 금지하는
 *    "안 잰 것이 통과로 세어지는" 자리다 — 빌드가 안 되면 여기서 빨개져야 한다.
 */
describe('빌드된 dist 도 같은 계약을 지킨다 — 소비 쪽이 실제로 부르는 것', () => {
  const DIST = join(ROOT, 'dist/lint/cli.js');

  beforeAll(() => {
    // 건너뛰는 대신 만든다. 실패하면 이 describe 전체가 빨개진다.
    execFileSync('npm', ['run', 'build'], { cwd: ROOT, encoding: 'utf8' });
    expect(existsSync(DIST), `빌드했는데도 ${DIST} 가 없다 — 소비 쪽 호출 경로가 깨졌다`).toBe(true);
  }, 60_000);

  const runNode = (script: string, args: string[]): IRun => {
    try {
      return {
        exitCode: 0,
        stdout: execFileSync(process.execPath, [script, ...args], { cwd: ROOT, encoding: 'utf8' }),
      };
    } catch (error) {
      const e = error as { status: number | null; stdout: string };
      return { exitCode: e.status ?? -1, stdout: e.stdout };
  ...
  it('스크립트 경로가 틀리면 stdout 이 비어 있다 — 소비 쪽 판별의 전제', () => {
    // 빌드해도 이 경로는 생기지 않는다. "클론만 하고 빌드 안 함" 과 같은 상태다.
    const { exitCode, stdout } = runNode(join(ROOT, 'dist/lint/NOPE.js'), ['--json']);
    expect(exitCode, 'node 는 스크립트가 없으면 127 이 아니라 1 을 낸다').toBe(1);
    expect(stdout).toBe('');
  });

  it('클론 자체가 없어도 마찬가지다 — 같은 exit 1, 빈 stdout', () => {
    const { exitCode, stdout } = runNode(join(ROOT, '../NOPE-REPO/dist/lint/cli.js'), ['--json']);
    expect(exitCode).toBe(1);
    expect(stdout).toBe('');
  });

  /** ⭐ 문제의 핵심: 아래 세 경우는 종료 코드만으로는 위 두 경우와 구별되지 않는다. */
  const DIST_PATHS: Array<{ name: string; args: string[]; expect: number }> = [
    { name: '통과', args: ['--format', 'sheet', '--json', 'tests/fixtures/sheet/good.json'], expect: 0 },
    {
      name: '위반 (exit 1 — 스크립트 없음과 같은 값)',
      args: ['--format', 'sheet', '--json', 'tests/fixtures/handoff/spec-dirty.json'],
      expect: 1,
    },
    { name: '못 쟀다', args: ['--format', 'sheet', '--json', './없는디렉토리'], expect: 3 },
  ];

  for (const { name, args, expect: expected } of DIST_PATHS) {
    it(`${name} → exit ${expected} 이고 stdout 이 유효 JSON`, () => {
      const { exitCode, stdout } = runNode(DIST, args);
      expect(exitCode).toBe(expected);
      expect(stdout.length, `${name}: dist 의 stdout 이 비었다 — 소비 쪽이 "도구 미실행"으로 오분류한다`)
        .toBeGreaterThan(0);
      const parsed = JSON.parse(stdout) as Record<string, unknown>;
      expect(parsed['tool']).toBe('tc-lint');
      expect(parsed['ran']).toBe(true);
