...
const SUBCOMMANDS = {
  ...
  /* **이 기계의 `gh` 가 아는 계정으로 레포를 고른다.** ⛔ 토큰을 인자로도 화면으로도 안 받는다 —
     `gh auth token` 을 부르지 않고, gh 의 말을 나를 때 **토큰처럼 생긴 것을 지운다.**
     ⛔ `gh auth login` 은 **사람이 한다** — 안 돼 있으면 ⚪ 못 쟀다(3)로 말한다. */
  repos: 'bin/repos.mjs',
  /* **깃 로그인을 화면에서 밟게 한다** — `status` 는 지금 계정을, `login` 은 기기 흐름을 낸다.
     ⛔⛔ 이것은 바로 위 `repos` 의 「`gh auth login` 은 **사람이 한다**」를 **일부러 뒤집은 자리**다.
        금지는 뒤집혔지만 **이유는 안 뒤집혔다**: 로그인 전/후 계정을 둘 다 찍고,
        **바뀌었으면 바뀌었다고** 말한다(개인 계정과 조직 계정이 섞이는 날 그것이 전부다).
     ⛔ 토큰 규율은 `repos` 와 같다 — 인자로도 화면으로도 안 받고, `gh auth token` 을 안 부른다. */
  'gh-auth': 'bin/gh-auth.mjs',
  /* **그 저장소에 어떤 가지가 있는가** — 묻기만 한다(`git ls-remote`). ⛔ 받아 오지 않는다:
     받는 것은 `clone` 한 자리다(두 자리에서 받으면 조용히 갈린다 — R47·R91).
     ⛔ 0개는 「없다」가 아니라 ⚪ 못 쟀다(3)다. */
  branches: 'bin/branches.mjs',
  /* **사람이 직접 로그인하고, 그 세션을 화면 시험이 물려받는다.**
     ⛔⛔ 계정을 **인자로도 화면으로도 안 받는다** — 창을 띄우고 **비켜선다.**
     ⛔ 대신 로그인하지 않는다: SSO·OTP 를 기계가 흉내 내려다 실패하면 **왜 실패했는지도 안 남는다.**
     ⚠️ 세션 없이 도는 화면 시험은 **셸만 잰다** — 그걸 「통과」라고 부르면 거짓 초록이다. */
  session: 'bin/session.mjs',
  /* **페이지별 아키텍처·메뉴 설명이 있는가 — 없으면 만든다.**
     ⛔ 기본은 **모델 호출 0회**다. `--write` 를 줘야 부르고, 부르기 전에 **몇 번 부를지**와
     「⚠️ 토큰 사용량이 큽니다」를 먼저 말한다(R145 — 「공짜인 줄 알고 돌렸다」가 이 저장소의 그 사건이다).
     ⛔ 설명이 없으면 ⚪ **못 쟀다**다 — 「구조가 없다」가 아니다. */
  blueprint: 'observatory/blueprint.mjs',
  clone: 'bin/clone.mjs',
  ...
  /* ⛔ 이것은 **관문이 부르는데 사람이 칠 수는 없었다.** 그런데 도구 자신은 화면에서
     `universe coordinates` 라고 **말하고 있었다** — 시키는 대로 치면 「모르는 명령」이 났다.
     도구가 화면에 적은 처방은 **그 도구가 실제로 지키는 것**이어야 한다.
     ⚠️ 짝이던 `qa`(흡수한 TC 도구)는 2026-09-09 에 화면과 함께 `feat/console-screens` 로 갔다. */
...
if (!command || wantsHelp) {
  ...
  for (const dir of (process.env.PATH ?? '').split(path.delimiter).filter(Boolean)) {
    /* ⛔ **여기의 `universe` 는 명령 이름이다 — 패키지 이름(`universe-front-harness`)이 아니다.**
   ...
   * `npm link` 가 전역에 등록하는 이름은 `package.json` 의 `name`(= `universe-front-harness`)이고,
   * PATH 에 놓이는 파일 이름은 `bin` 의 키(= `universe`)다. **이 둘이 다르다.**
   * ⚠️ 예전엔 둘이 같아서(`name: "universe"`) `npm rm -g universe` 가 우연히 맞았다.
   * npm 에 올리며 패키지 이름만 `universe-front-harness` 로 바뀌었으므로, 옛 문구를 그대로 두면
  ...
  lines.push('', how === 'universe'
    ...
    : `  ⚠️ \`universe\` 는 아직 PATH 에 없다 — 지금은 \`${how} …\` 로 부른다.\n`
      + `     이어 붙이려면: cd ${packageHome} && npm link   (되돌리기: npm rm -g universe-front-harness)\n`
