 ...
 *
 *   universe clone <주소> [--into <받을 자리>] [--name <은하 이름>] [--out <좌표 파일>]
 *
...
rejectUnknownFlags(argv, ['--universe', '--into', '--name', '--out'], 'universe clone');
const flag = (n) => (argv.includes(n) ? argv[argv.indexOf(n) + 1] : undefined);

const taken = new Set(['--into', '--name', '--out', '--universe']
  .flatMap((f) => (argv.includes(f) ? [argv[argv.indexOf(f) + 1]] : [])));
const url = argv.find((a) => !a.startsWith('--') && !taken.has(a));

if (!url) {
  console.error('주소를 줘라: universe clone <주소> [--into <받을 자리>] [--name <은하 이름>]');
...
const code = await run('git', ['clone', '--', url, into]);
if (code !== 0) {
