 ...
 *   ⚠️⚠️ **이 금지는 `bin/gh-auth.mjs` 에서 일부러 뒤집혔다** — 화면에서 로그인을 밟게 해 달라는
 *     요구가 있었고, 터미널을 아는 사람만 밟는 칸은 이 제품의 전제와 어긋난다.
 *     ⛔ **이 도구는 그대로 안 띄운다**(둘이 띄우면 코드가 둘 생긴다). 뒤집힌 것은 금지이고
 *     **이유는 안 뒤집혔다**: 그쪽은 로그인 전/후 계정을 둘 다 찍고 **바뀌면 바뀌었다고 말한다.**
