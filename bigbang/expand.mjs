...
export const EXIT = {
  ...
  gateDead: 2,
  /**
   * **재라고 한 것을 못 쟀다.** 게이트는 지났지만 사람이 요청한 측정이 안 이뤄졌다(R158).
   *
   * ⚠️⚠️ 왜 0 이 아닌가: `--contract-first` 를 켜는 것은 「요구사항을 재 달라」는 말이다.
   * 그런데 계약을 못 받으면 예전엔 **조용히 그냥 진행**했고, 화면은 평범한 초록불 주행처럼 보였다 —
   * **재 달라고 했는데 안 재고 통과로 보이는 것**, §8 이 계속 잡아 온 바로 그 모양이다.
   * ⛔ 실패(1)도 아니다 — 별은 게이트를 지났다. 그래서 **셋째 자리**가 필요하다.
   * (관문의 `EXIT_UNMEASURED` 와 같은 뜻·같은 값이다 — 두 층이 같은 말을 쓴다.)
   */
  unmeasured: 3,
};
