...
```
정책(저장소 문서 · hail-mary 의 Confluence/Jira) ──▶ TC 티켓 ──▶ 자동 테스트(Playwright) ──▶ 결과
```

이 저장소는 2026-09-10 에 비워지고 다시 시작했다. 이전 것(프론트엔드 에이전트 훈련장 「우주」)은
가지 `archive/universe-2026-09-10` 과 태그 `universe-last` 에 그대로 있다.
`qa-harness`(정책서 → TC 티켓 → Playwright 의 앞선 시도)를 2026-09-11 에 이력째 흡수했다 — 아래 「TC 관문」이 그것이다.
...
- 저장소 안 문서 — 먼저 여기로 파이프라인을 세운다.
- `project-hail-mary` MCP — 사내 정책·스펙·과거 티켓을 도구로 준다. 출처 하나로 나중에 붙인다.

## 대상 앱

작은 픽스처 앱부터 세우고, 실제 앱은 그 뒤다.

---

# TC 관문 (qa-harness 에서 흡수)
...
(실측: 지식 폴더 10개 중 7개가 NFD였고 `목록관리` 는 NFC 4자 / NFD 11자다).
